...
kao zahtjev \textit{Nesigurnog} gosta za čitanje i pisanje sigurne memorije. Izmijenjene funkcije za pristup SLCR-u prikazane su slikom \ref{slcr_secure}.
...
\textit{bare-metal} aplikacije je što postoji mogućnost za istiskivanje zadataka. Ova karakteristika operacijskom sustavu
pruža da zadaci koji su prioritetniji prije obave potrebni posao od onih koji su nižeg prioriteta što nije moguće kod
...
registrom, kao i početna inicijalizacija konteksta zadatka (početni sadržaj stoga zadatka). Isto tako, datoteka sadrži i
...
kritična sekcija (nužno je da funkcija ne bude prekinuta). U svakom trenutku izvođenja FreeRTOS aplikacije mora biti aktivan
...
softverski konfigurirati te indicira na to da se FIQ ne može softverski maskirati. Poznato je da se FIQ automatski maskiraju
generiranje FIQ prekida te se ta činjenica iskoristila za implementaciju kritične sekcije potrebne za
...
funkcioniranje SMP Linuxa. Linux prati preporuke ARM-a za \textit{Nesigurni} svijet te koristi SGI ID0~-~ID7. Navedeni prekidi
...
\textit{zynq\_cpun\_start} funkcije točno ona adresa koju je potrebno upisati na \textit{0xFFFFFFF0}.\\
...
\begin{enumerate}
  ...
  \item{Na adresu \textit{0x00000000} se kopira minimalni kod koji služi za skok na \textit{\&secondary\_startup} adresu}
  \item{Čisti se \textit{cache} memorija u memorijskom području koje obuhvaća kopirani kod s početnom adresom
  \textit{0x00000000} (nužno za očuvanje koherencije)}
...
\textit{0xFFFFFFF0} lokaciju mora izvesti u \textit{Sigurnom} stanju procesora (lokacija je nedostupna \textit{Nesigurnom}
svijetu). Postupak kojim je izvedeno pokretanje sekundarne jezgre u \textit{Nesigurnom} gostu LTZVisora je:
\begin{enumerate}
  \item {Linux generira zahtjev LTZVisoru za upis odgovarajuće adrese na \textit{0xFFFFFFF0} lokaciju}
...
\end{enumerate}
U ovom slučaju nema potrebe za rukovanjem \textit{cache} memorijom jer se upis na \textit{0xFFFFFFF0} lokaciju izvodi
