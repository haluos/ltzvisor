...
Kao što je već napomenuto, prilikom prilagođenja Linuxa za izvršavanje u Nesigurnom svijetu treba voditi posebnu brigu o
pristupu Linuxa hardveru. Da bi uopće imalo smisla tražiti dijelove izvornog koda koji bi mogli biti problematični za
Nesigurni svijet, jasno je da programer mora biti vrlo dobro upoznat arhitekturom \textit{TrustZone} ekstenzije, kao i
...
potrebne promjene (SLCR je dostupan samo Sigurnom svijetu, a nužan je za npr. podešavanje signala takta za razne uređaje).
...
kao zahtjev Nesigurnog gosta za čitanje i pisanje sigurne memorije. Izmijenjene funkcije za pristup SLCR-u prikazane su slikom
\ref{slcr_secure}.
\begin{figure}[H]
...
upisivati samo u Sigurnom svijetu. Za uspješno konfiguriranje registra za upravljanje snagom također je implementirana posebna
...
samo Sigurnom svijetu, ali nema potrebe za implementacijom posebnog poziva LTZVisoru za upis u ACTLR jer su Linuxovi upisi u
registar zadovoljeni postavljanjem početnog konteksta Nesigurnog gosta.
...
Navedene promjene su minimalne za uspješno pokretanje operacijskog sustava Linux kao Nesigurnog gosta LTZVisora. Iako se Linux
...
Tree} strukturi. Naime, \textit{PetaLinux} prilikom generiranja \textit{Device Tree} strukture pretpostavlja Sigurno stanje i
time automatski opisuje DMAC koji pripada Sigurnom svijetu (Zynq-7000 platforme imaju poseban DMAC za Sigurni i poseban za
Nesigurni svijet čije se adrese razlikuju). Da se ukloni takva pogreška, potrebno je u \textit{Device Tree} umjesto DMAC
Sigurnog svijeta, opisati DMAC Nesigurnog svijeta (prvenstveno se odnosi na promjenu bazne adrese upravitelja).
Adresa DMA upravitelja Nesigurnog svijeta je 0xf8004000.
...
Uz izmjene Linuxa, potrebne su i izmjene u LTZVisoru koje se odnose na pripremu hardverskih resursa za korištenje u Nesigurnom
...
koji memorijski segmenti pripadaju Sigurnom, a koji Nesigurnom svijetu). Navedena inicijalizacija platforme se odnosi na
...
i NSACR (engl. \textit{Non-Secure Access Control Register}) koji služe za konfiguraciju dozvole pristupa Nesigurnog svijeta
privatnom i globalnom brojilu.\\
Dodatna promjena se odnosi na podešavanje DMA upravitelja, odnosno označavanje DMA upravitelja da će se koristiti u Nesigurnom
...
Uz navedene promjene, potrebno je i označiti prekid globalnog brojila kao IRQ, odnosno kao prekid Nesigurnog svijeta što se
